...
def load_instr_datasets(anno_dir, dataset, splits, args=None):
    ...
    for split in splits:
        if "/" not in split:    # the official splits
            if dataset == 'r2r':
                with open(os.path.join(anno_dir, 'R2R_%s_landmark_co-occurrence_chatgpt_enc.json' % split)) as f:
                ...
                with open(os.path.join(anno_dir, 'R4R_%s_landmark_co-occurrence_chatgpt_enc.json' % split)) as f:
                    new_data = json.load(f)
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits,args=args)):
        if dataset == 'rxr':
            ...
            for j, instr in enumerate(item['instructions']):
                ...
                new_item['landmark'] = None
                new_item['cooccurrence_landmark'] = None
                new_item['cooccu_landmark_enc'] = None

                if 'landmark_list' in list(item.keys()):
                    if len(item['landmark_list'][j]) == 0:
                        new_item['landmark'] = ['room']
                        new_item['cooccurrence_landmark'] = {
                            "room": ["door","window","walls","ceiling","floor",
                                     "furniture","lighting","decorations","carpet","curtains"]}
                    else:
                        new_item['landmark'] = item['landmark_list'][j]
                        new_item['cooccurrence_landmark'] = item['co-occurrence_landmark'][j]
                elif 'landmark' in list(item.keys()):
                    if len(item['landmark'][j]) == 0:
                        new_item['landmark'] = ['room']
                        new_item['cooccurrence_landmark'] = {
                            "room": ["door","window","walls","ceiling","floor",
                                     "furniture","lighting","decorations","carpet","curtains"]}
                    else:
                        new_item['landmark'] = item['landmark'][j]
                        new_item['cooccurrence_landmark'] = item['co-occurrence_landmark'][j]


                co_occurrence_list_item = []
                for land_ind in range(len(new_item['landmark'])):

                    key = new_item['landmark'][land_ind]
                    value = new_item['cooccurrence_landmark'][new_item['landmark'][land_ind]]

                    if len(value) == 0:
                        value = [key] * args.cooccurrence_num
                    else:
                        value.extend([key] * (args.cooccurrence_num-len(value)))
                    if args.cooccurrence_num > 0:

                        landmark_cooccu_list=value[:args.cooccurrence_num]

                    else:
                        landmark_cooccu_list = []
                    landmark_cooccu_list.append(key)

                    co_occurrence_list_item.extend(landmark_cooccu_list)

                assert len(co_occurrence_list_item) == len(new_item['landmark']) * (
                            args.cooccurrence_num + 1)
                new_item['cooccu_landmark_enc'] = co_occurrence_list_item

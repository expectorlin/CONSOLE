...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, feat_db_two=None,batch_size=100):
        """
    ...
    def getStates(self):
        ...
        for i, sim in enumerate(self.sims):
            ...
            if self.feat_db_two is not None:
                feature_two = self.feat_db_two.get_image_feature(state.scanId, state.location.viewpointId)
                feature_states.append((feature, feature_two, state))
            else:
...
class R2RBatch(object):
    ...
    def __init__(
        self, feat_db, feat_db_two,instr_data, connectivity_dir,
        batch_size=64, angle_feat_size=4,
        seed=0, name=None, sel_data_idxs=None,args=None
    ):

        self.args = args
        self.env = EnvBatch(connectivity_dir, feat_db=feat_db,feat_db_two=feat_db_two, batch_size=batch_size)
    ...
    def make_candidate(self, feature, feature_two,scanId, viewpointId, viewId):
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                if feature_two is not None:
                    visual_feat_two = feature_two[ix]
                    ...
                    if feature_two is not None:
                        if (loc.viewpointId not in adj_dict or
                            ...
                            adj_dict[loc.viewpointId] = {
                                ...
                                'distance': distance,
                                'idx': j + 1,
                                'feature': np.concatenate((visual_feat, visual_feat_two,angle_feat), -1),
            ...
            for c in candidate:
                ...
                if feature_two is not None:
                    c_new['feature'] = np.concatenate((visual_feat, visual_feat_two, angle_feat), -1)
                else:
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, feature_two,state) in enumerate(self.env.getStates()):
            item = self.batch[i]
            ...
            # Full features
            candidate = self.make_candidate(feature, feature_two,state.scanId, state.location.viewpointId, state.viewIndex)
            # [visual_feature, angle_feature] for views
            feature = np.concatenate((feature, feature_two,self.angle_feature[base_view_id]), -1)
            ...
            obs.append({
                ...
                'cooccurrence_landmark': item['cooccurrence_landmark'],
                'cooccu_landmark_enc': item['cooccu_landmark_enc'],
            })

            if 'instr_encoding' in item:
                obs[-1]['instr_encoding'] = item['instr_encoding']
            # A2C reward. The negative distance between the state and the final state
            obs[-1]['distance'] = self.shortest_distances[state.scanId][state.location.viewpointId][
                item['path'][-1]]

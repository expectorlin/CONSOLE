...
def build_dataset(args, rank=0, is_test=False):
    ...
    feat_db_two = ImageFeaturesDB(os.path.join(args.root_dir, 'R2R', 'features', 'pth_vit_base_patch32_224_clip.hdf5'), 512)
    ...
    train_instr_data = construct_instrs(
        ...
        max_instr_len=args.max_instr_len, args=args
    )
    train_env = dataset_class(
        feat_db, feat_db_two, train_instr_data, args.connectivity_dir, batch_size=args.batch_size,
    ...
    if args.aug is not None:
        ...
        aug_env = dataset_class(
            feat_db, feat_db_two,aug_instr_data, args.connectivity_dir, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db,feat_db_two, val_instr_data, args.connectivity_dir, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter+args.iters, args.log_every):
        ...
        if default_gpu:
            ...
            contrastive_loss = sum(listner.logs['contrastive_loss']) / max(len(listner.logs['contrastive_loss']), 1)
            writer.add_scalar("loss/contrastive_loss", contrastive_loss, idx)
            ...
            write_to_record_file(
                "\ntotal_actions %d, max_length %d, entropy %.4f, IL_loss %.4f, RL_loss %.4f, consistency_loss %.4f,contrastive_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
                    total, length, entropy, IL_loss, RL_loss, consistency_loss, contrastive_loss,policy_loss, critic_loss),
                record_file
            )

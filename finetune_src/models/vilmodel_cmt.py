...
class ImageEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.txt_linear = nn.Linear(512, config.hidden_size)
        self.txt_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)

        self.args = config.args

    def forward(self, img_feat, ang_feat, type_embeddings, nav_types=None, mode=None, landmark_feat=None):
        if mode == 'text':
